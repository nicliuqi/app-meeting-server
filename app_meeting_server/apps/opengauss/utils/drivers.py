...
from opengauss.utils.welink_apis import getParticipants as weLinkGetParticipants
from opengauss.utils.welink_apis import updateMeeting as weLinkUpdateMeeting
...
def updateMeeting(mid, date, start, end, topic, record):
    ...
    if platform == 'zoom':
        ...
        status = weLinkUpdateMeeting(mid, date, start, end, topic, record)
    return status
...
def getParticipants(mid):
    ...
    if mplatform == 'zoom':
        ...
        status, res = weLinkGetParticipants(mid)
    return status, res
