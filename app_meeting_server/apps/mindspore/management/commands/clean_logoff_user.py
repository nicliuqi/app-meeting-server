...
from mindspore.models import User, GroupUser, Meeting, Collect, Activity, ActivityCollect, Record, CityUser
...
class Command(BaseCommand):
    def delete_user_data(self, user_id):
        with transaction.atomic():
            # 1.delete group user and city user
            ret = GroupUser.objects.filter(user_id=user_id).delete()
            logger.info("delete groupuser and result is:{}".format(str(ret)))
            ret = CityUser.objects.filter(user_id=user_id).delete()
            logger.info("delete city user and result is:{}".format(str(ret)))
            # 2.delete meetings
    ...
    def clear_expired_activity_data(self):
        ...
        for activitys in expired_activitys:
            ...
            logger.info("The activitys(activitys_id:{}) create time over expired date, start to delete".format(str(activitys_id)))
